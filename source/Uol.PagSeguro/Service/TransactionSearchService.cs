...
namespace Uol.PagSeguro.Service
{
    ...
    public static class TransactionSearchService
    {
        ...
        public static Transaction SearchByCode(Credentials credentials, string transactionCode)
        {
            ...
            try
            {
                using (var response = HttpURLConnectionUtil.GetHttpGetConnection(BuildSearchUrlByCode(credentials, transactionCode), credentials.IsSandbox()))
                {
        ...
        public static TransactionSearchResult SearchByReference(Credentials credentials, string reference)
        {
            ...
            try
            {
                using (var response = HttpURLConnectionUtil.GetHttpGetConnection(BuildSearchUrlByReference(credentials, reference), credentials.IsSandbox()))
                {
                    using (var reader = XmlReader.Create(response.GetResponseStream()))
                    {
        ...
        public static TransactionSearchResult SearchAbandoned(Credentials credentials, DateTime initialDate, DateTime finalDate, int? pageNumber = null, int? resultsPerPage = null)
        {
            ...
            try
            {
                using (var response = HttpURLConnectionUtil.GetHttpGetConnection(BuildSearchUrlAbandoned(credentials, initialDate, finalDate, pageNumber, resultsPerPage), credentials.IsSandbox()))
                {
                    using (var reader = XmlReader.Create(response.GetResponseStream()))
                    {
                        var result = new TransactionSearchResult();
